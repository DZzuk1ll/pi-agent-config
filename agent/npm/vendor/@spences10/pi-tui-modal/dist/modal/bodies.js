import { fuzzyFilter, getKeybindings, Input, Key, matchesKey, truncateToWidth, visibleWidth, wrapTextWithAnsi, } from '@earendil-works/pi-tui';
import { cycle_setting_value, normalize_text } from './layout.js';
function expand_tabs(text) {
    return text.replace(/\t/g, '   ');
}
export class TextModalBody {
    text;
    max_visible_lines;
    theme;
    on_cancel;
    ...
    render(width) {
        this.wrapped_lines = normalize_text(this.text).flatMap((block) => expand_tabs(block)
            .split('\n')
...
export class DetailedSettingsList {
    items;
    max_visible;
    theme;
    on_change;
    ...
    handleInput(data) {
        ...
        else if (!this.search_input?.getValue() &&
    ...
    add_hint_line(lines, width) {
        ...
        lines.push(truncateToWidth(this.theme.hint(this.search_input
            ...
            : '  ←/→ change · Enter/Space next · Esc cancel'), width));
    }
}

import { Box, Key, matchesKey, SelectList, SettingsList, Text, } from '@earendil-works/pi-tui';
import { DetailedSettingsList, InputModalBody, TextModalBody, } from './bodies.js';
import { render_framed_modal } from './frame.js';
import { cycle_setting_value, default_overlay_options, fit_visible_items, get_modal_body_line_budget, get_selected_setting, has_active_settings_search, is_focusable, make_select_theme, make_settings_theme, normalize_metadata, normalize_text, set_component_max_visible, } from './layout.js';
...
export async function show_picker_modal(ctx, options) {
    ...
    let selected_item = options.items[options.initial_index ?? 0];
    ...
    return await show_modal(ctx, {
        ...
        footer: options.selected_footer
            ? () => {
                const selected_footer = options.selected_footer?.(selected_item);
                if (selected_footer !== undefined)
                    return selected_footer;
...
export async function show_command_output_modal(ctx, options) {
...
export async function show_settings_modal(ctx, options) {
    await show_modal(ctx, {
        ...
        return {
            ...
            handleInput: (data) => {
                if (list instanceof SettingsList &&
                    !has_active_settings_search(list) &&
                    (matchesKey(data, Key.left) ||
                        matchesKey(data, Key.right))) {
                    const item = get_selected_setting(list);
                    const new_value = cycle_setting_value(item, matchesKey(data, Key.left) ? -1 : 1);

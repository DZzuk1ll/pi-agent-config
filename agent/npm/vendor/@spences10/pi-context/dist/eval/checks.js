...
export function run_retention_evals(store, results) {
    ...
    with_sqlite_busy_retry(() => store_db(store)
        .prepare('UPDATE context_sources SET created_at = ? WHERE id = ?')
        .run(Date.now() - 10 * 24 * 60 * 60 * 1000, old.source_id), { operation: 'Seed context retention eval' });
...
export function run_dedupe_evals(results) {
    ...
    try {
        ...
        const same_session_chunks = store.get(first.source_id, undefined, {
            session_id: `${SESSION}:a`,
            project_path: PROJECT,
        });
        const same_session_passed = first.source_id === same_session.source_id &&
        ...
        results.push({
            ...
            description: 'Identical content in the same default retrieval scope should reuse the existing source.',
        ...
        const other_session_chunks = store.get(other_session.source_id, undefined, {
            session_id: `${SESSION}:b`,
            project_path: PROJECT,
        });
        const hidden_from_first_session = store.get(other_session.source_id, undefined, {
            session_id: `${SESSION}:a`,
            project_path: PROJECT,
        });
        const stats = store.stats({ global: true });
        const cross_session_passed = other_session.source_id !== first.source_id &&
        ...
        results.push({
            ...
            description: 'Identical content across sessions should remain separately scoped and visible only from its own retrieval scope.',

...
import { command_may_create_temp_path, extract_bash_create_paths, extract_created_temp_paths_from_result, } from './destructive/shell.js';
...
export default async function confirm_destructive(pi) {
    ...
    const pending_created_files = new Map();
    const pending_bash_created_paths = new Map();
    const bash_may_create_temp_path = new Set();
    const session_created_files = new Set();
    ...
    pi.on('tool_call', async (event, ctx) => {
        ...
        const action = assess_tool_call(event, ctx.cwd, session_created_files);
        if (!action)
            return;
    ...
    pi.on('tool_result', async (event) => {
        ...
        const bash_paths = pending_bash_created_paths.get(event.toolCallId);
        if (bash_paths) {
        ...
        if (bash_may_create_temp_path.has(event.toolCallId)) {
            ...
            if (event.toolName === 'bash' && !event.isError) {
                for (const path of extract_created_temp_paths_from_result(event)) {
                    session_created_files.add(path);
                }
            }
        }
    ...
    pi.on('user_bash', async (event, ctx) => {
        const action = assess_bash_command(event.command, event.cwd, session_created_files);
        if (!action)
            return;
        if (await should_allow(action, ctx))
            return;

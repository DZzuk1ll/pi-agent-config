...
function is_path_within(parent, child) {
    const rel = relative(parent, child);
    return Boolean(rel) && !rel.startsWith('..') && !isAbsolute(rel);
}
export function is_temp_path(path) {
    const temp_root = resolve(tmpdir());
    const absolute = resolve(path);
    return (absolute === temp_root || is_path_within(temp_root, absolute));
}
export function is_agent_temp_path(path) {
    if (!is_temp_path(path))
        return false;
    const temp_root = resolve(tmpdir());
    const first_segment = relative(temp_root, resolve(path)).split(/[\\/]+/)[0];
    return /^my-pi-(audit|sandbox|temp|tmp|work|session)-/.test(first_segment);
}
export function describe_path_risk(cwd, paths) {

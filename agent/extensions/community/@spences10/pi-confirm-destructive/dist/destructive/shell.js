...
function heredoc_delimiters(line) {
    const delimiters = [];
    ...
    for (let index = 0; index < line.length; index += 1) {
        ...
        for (; index < line.length; index += 1) {
            ...
            if (/\s/.test(delimiter_character) ||
                ';&|()<>'.includes(delimiter_character))
                break;
            value += delimiter_character;
        }
        if (value)
            delimiters.push({ value, strip_tabs });
    }
    return delimiters;
}
function strip_heredoc_bodies(command) {
    const pending = [];
    return command
        .split(/\r?\n/)
        .map((line) => {
...
function tokenize_shell(command) {
    const tokens = [];
    let word = '';
    const flush_word = () => {
        if (!word)
...
function skip_options(words, start, options_with_values = new Set()) {
    let index = start;
...
function unwrap_command(words) {
    ...
    for (;;) {
        ...
        if (!current)
            return undefined;
        if (current === 'sudo') {
            index = skip_options(words, index + 1, new Set([
                '-C',
                '-D',
                '-g',
                '-h',
        ...
        if (current === 'nice') {
            index = skip_options(words, index + 1, new Set(['-n', '--adjustment']));
            continue;
        }
        if (current === 'ionice') {
            index = skip_options(words, index + 1, new Set([
        ...
        if (current === 'stdbuf') {
            index = skip_options(words, index + 1, new Set(['-e', '-i', '-o', '--error', '--input', '--output']));
            continue;
        }
        ...
        if (current === 'git') {
            ...
            const command_index = skip_options(args, 0, new Set([
                '-C',
                '-c',
                '--config-env',
                '--exec-path',
...
function xargs_command(args) {
    let index = skip_options(args, 0, new Set(['-a', '-d', '-E', '-I', '-L', '-n', '-P', '-s']));
    while (index < args.length && is_assignment(args[index]))
        index += 1;
    return unwrap_command(args.slice(index));
}
function nested_find_commands(args) {
    const nested = [];
    for (let index = 0; index < args.length; index += 1) {
        if (args[index] !== '-exec' && args[index] !== '-execdir')
            continue;
        const end = args.findIndex((word, candidate) => candidate > index && (word === ';' || word === '+'));
        const words = args.slice(index + 1, end === -1 ? undefined : end);
        const invocation = unwrap_command(words);
        if (invocation)
            nested.push(...expand_invocation(invocation));
        if (end !== -1)
            index = end;
    }
    return nested;
}
function expand_invocation(invocation) {
    const invocations = [invocation];
    if (SHELL_COMMANDS.has(invocation.command)) {
        const command_index = invocation.args.findIndex((arg) => arg === '--command' || /^-[^-]*c/.test(arg));
    ...
    if (invocation.command === 'eval' && invocation.args.length > 0) {
        invocations.push(...extract_shell_invocations(invocation.args.join(' ')));
    }
    if (invocation.command === 'xargs') {
        const nested = xargs_command(invocation.args);
        if (nested)
            invocations.push(...expand_invocation(nested));
    }
...
function invocations_from_segment(segment) {
    const words = segment
...
function embedded_command_texts(command) {
    const embedded = [];
    let single_quoted = false;
    let double_quoted = false;
    for (let index = 0; index < command.length; index += 1) {
...
export function extract_shell_invocations(command) {
...
function paths_from_invocation(invocation) {
    const offset = invocation.command === 'git' && invocation.args[0] === 'rm'
        ? 1
        : 0;
...
export function extract_command_paths(command, command_name_to_find) {
    const invocations = extract_shell_invocations(command).filter((invocation) => command_name_to_find === 'rm'
...
export function extract_overwrite_paths(command) {
...
function option_takes_value(command_name_to_check, option) {
    const value_options = {
        mkdir: ['-m', '--mode', '-Z', '--context'],
        touch: ['-r', '--reference', '-d', '--date', '-t'],
    };
    return (value_options[command_name_to_check]?.includes(option) ?? false);
}
function extract_simple_create_paths(command, cwd) {
    const invocations = extract_shell_invocations(command);
    const paths = [];
    for (const invocation of invocations) {
        if (!['mkdir', 'touch'].includes(invocation.command))
            continue;
        for (let index = 0; index < invocation.args.length; index += 1) {
            ...
            if (!word || word === '--')
                continue;
            if (word.startsWith('-')) {
                if (option_takes_value(invocation.command, word))
                    index += 1;
                continue;
            }
...
function extract_redirect_create_paths(command, cwd) {
    return extract_overwrite_paths(command)
        .map((path) => resolve(cwd, path))
        .filter((path) => is_temp_path(path) && !existsSync(path));
}
export function extract_bash_create_paths(command, cwd) {
    return [
        ...extract_simple_create_paths(command, cwd),
        ...extract_redirect_create_paths(command, cwd),
    ];
}
export function command_may_create_temp_path(command) {
    return extract_shell_invocations(command).some((invocation) => invocation.command === 'mktemp');
}
function text_content(event) {
...
export function extract_created_temp_paths_from_result(event) {

...
export function relaxed_fts5_query(query) {
    const normalized = normalized_fts_tokens(query);
    if (normalized.length < 2)
        return null;
    const tokens = normalized
        .flatMap((token) => token.value.split(/\s+/).map((value) => ({
        value,
        prefix: token.prefix,
    })))
...
function normalized_fts_tokens(query) {
    return query
        .trim()
        .split(/\s+/)
...
export function chunk_text(text, source_id) {
...
function split_lossless_chunks(text, target_bytes) {
    const chunks = [];
    let rest = text;
...
function preferred_break_index(head) {
...
function last_paragraph_break_index(text) {
    let index = 0;
...
export function summarize_source(result, tool_name) {
...
function format_capture_reason(result) {
    if (result.capture_max_bytes === undefined ||
        result.capture_max_lines === undefined)
        return undefined;
    const reasons = [];
    if (result.bytes > result.capture_max_bytes) {
        reasons.push(`${format_bytes(result.bytes)} exceeds ${format_bytes(result.capture_max_bytes)}`);
    }
    if (result.lines > result.capture_max_lines) {
        reasons.push(`${result.lines} lines exceeds ${result.capture_max_lines}`);
    }
    if (reasons.length === 0) {
        reasons.push(`forced capture; thresholds are ${format_bytes(result.capture_max_bytes)} or ${result.capture_max_lines} lines`);
    }

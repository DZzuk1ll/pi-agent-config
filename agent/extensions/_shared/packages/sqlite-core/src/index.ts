...
export function sqlite_pragmas(options: { foreign_keys?: boolean; busy_timeout_ms?: number } = {}): { persistent: string; connection: string } {
	const foreign_keys = options.foreign_keys ?? true;
	const busy_timeout_ms = options.busy_timeout_ms ?? DEFAULT_SQLITE_BUSY_TIMEOUT_MS;
	return {
...
export function format_sqlite_busy_message(operation = "SQLite operation"): string {
...
export function with_sqlite_busy_retry<T>(
	fn: () => T,
	options: { attempts?: number; delay_ms?: number; operation?: string; on_busy?: (attempt: number, error: unknown) => void } = {},
): T {
	...
	throw new SqliteBusyError(options.operation ?? "SQLite operation", last_error);
}

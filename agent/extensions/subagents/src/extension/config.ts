...
export function getConfigPath(): string {
	return path.join(getAgentDir(), "extensions", "subagents", "config.json");
}
...
function getReadableConfigPath(): string {
	const configPath = getConfigPath();
	return fs.existsSync(configPath) ? configPath : getLegacyConfigPath();
}
...
export function updateConfig(updater: (config: ExtensionConfig) => ExtensionConfig): ExtensionConfig {
	const configPath = getConfigPath();
	const next = updater(readConfigForUpdate(getReadableConfigPath()));

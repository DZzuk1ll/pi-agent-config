...
export const PLAN_MODE_SETTINGS_FILE = "config.json";
const PLAN_MODE_SETTINGS_RELATIVE_PATH = join("extensions", "plan-mode", PLAN_MODE_SETTINGS_FILE);
const PREVIOUS_PLAN_MODE_SETTINGS_FILE = "pi-plan-mode.json";
...
export async function readPlanModeSettings(
	...
	const canonicalPath = join(agentDir, PLAN_MODE_SETTINGS_RELATIVE_PATH);
	const canonical = await readSettingsFile(canonicalPath);
	const fallbacks = [PREVIOUS_PLAN_MODE_SETTINGS_FILE, LEGACY_PLAN_MODE_SETTINGS_FILE]
		.map((name) => ({ name, path: join(agentDir, name) }));

	if (canonical.kind !== "missing") {
		const existingFallback = await firstExistingFallback(fallbacks);
		return existingFallback
			? {
					...canonical,
					notice: `${existingFallback.name} ignored because ${PLAN_MODE_SETTINGS_RELATIVE_PATH} takes precedence.`,
				}
			: canonical;
	}

	let fallback: { name: string; path: string; snapshot: Awaited<ReturnType<typeof readSettingsSnapshot>> } | undefined;
	for (const candidate of fallbacks) {
		const snapshot = await readSettingsSnapshot(candidate.path);
		if (snapshot.result.kind !== "missing") {
	...
	const fallbackResult = fallback.snapshot.result;
	const raced = await readSettingsFile(canonicalPath);
	if (raced.kind !== "missing") return raced;
	if (fallbackResult.kind !== "loaded") return fallbackResult;

	let installedIdentity: FileIdentity;
	try {
		installedIdentity = await installFileExclusively(canonicalPath, fallback.snapshot.contents ?? "");
	} catch (error) {
		const created = await readSettingsFile(canonicalPath);
		if (created.kind !== "missing") {
			return {
				...created,
				notice: `${fallback.name} ignored because ${PLAN_MODE_SETTINGS_RELATIVE_PATH} was created concurrently.`,
			};
		}
		return {
			...fallbackResult,
			notice: `Plan-mode settings migration failed: ${formatError(error)}. ${fallback.name} was used for this session.`,
		};
	}

	if (!(await fileContentsEqual(fallback.path, fallback.snapshot.contents ?? ""))) {
		const removed = await removeFileIfIdentityMatches(
			canonicalPath,
			installedIdentity,
			fallback.snapshot.contents ?? "",
		);
		return {
			...fallbackResult,
			notice: removed
				? `${fallback.name} changed during migration; the stale ${PLAN_MODE_SETTINGS_RELATIVE_PATH} snapshot was removed.`
				: `${fallback.name} changed during migration, but ${PLAN_MODE_SETTINGS_RELATIVE_PATH} was replaced concurrently and takes precedence on the next load.`,
		};
	}

	try {
		await rm(fallback.path);
		return {
			...fallbackResult,
			notice: `Plan-mode settings migrated from ${fallback.name} to ${PLAN_MODE_SETTINGS_RELATIVE_PATH}.`,
		};
	} catch (error) {
		return {
			...fallbackResult,
			notice: `Plan-mode settings migrated to ${PLAN_MODE_SETTINGS_RELATIVE_PATH}, but ${fallback.name} could not be removed: ${formatError(error)}.`,
		};
	}
}

async function firstExistingFallback(
	fallbacks: Array<{ name: string; path: string }>,
): Promise<{ name: string; path: string } | undefined> {
	for (const fallback of fallbacks) {
		if (await exists(fallback.path)) return fallback;
	}
	return undefined;
}

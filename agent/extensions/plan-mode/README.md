...
> **Local fork:** this directory vendors the 0.41.0 source. Local integration keeps settings at `config.json`, uses the vendored `@personal-pi/tui-kit`, preserves the complete-plan Markdown renderer under Claudify, pre-fills `/plan` after completion, and adds exact-plan execution in a linked new session.
...
- Keeps extension and custom tools opt-in by policy; this local configuration explicitly opts into Codex Search and read-only LSP tools.
...
- Presents the complete plan, pre-fills `/plan`, and lets you implement in the current session or a linked new session.
...
This fork requires Pi 0.80.6 or newer and is auto-discovered from
`~/.pi/agent/extensions/plan-mode/index.ts`. Its TUI dependency is also vendored and linked from the
agent package as `@personal-pi/tui-kit`; this configuration does not install the upstream npm
...
question, `/plan implement` hands a stored plan to a normal implementation turn in the current
session, and `/plan implement-new` creates a linked replacement session and hands it the exact plan.
`show`, `implement`, and `implement-new` fail closed when no applicable plan is stored; `finalize`
...
Plan mode always includes `read`, limited `bash`, available read-only built-ins such as `grep`, `find`, and `ls`, plus the required `plan_mode_question` and `plan_mode_complete` tools. This local `config.json` also opts into `codex_search` and the available read-only LSP tools. Built-in `edit` and `write` are blocked. `update_plan` is also blocked because it tracks execution progress rather than conversational planning. Other extension and custom tools remain disabled by default because Pi tools do not expose standardized mutability metadata; enable them from `/plan tools` only when you accept the risk for that session.
...
`plan_mode_complete` uses Pi's `terminate: true` hint. Termination is best effort: if a model puts it in a parallel tool batch, Pi terminates the batch early only when every finalized sibling tool also terminates. The prompt therefore requires the completion call to be standalone and last. The extension deliberately does not infer completion from phrases such as “I will present the plan.” A normal turn without a completion signal stays active and reports that no plan was submitted. After an explicit `/plan finalize`, a missing signal triggers one hidden recovery turn; a second miss stops automatically, keeps Plan mode active, and asks the user to retry `/plan finalize`.
...
Edit `$PI_CODING_AGENT_DIR/extensions/plan-mode/config.json` (normally `~/.pi/agent/extensions/plan-mode/config.json`) to configure this local fork. The file is read at session start. A valid root-level `pi-plan-mode.json` or legacy `plan-mode.json` is migrated into this nested path when the nested file is missing.
...
Invalid settings produce a warning and fall back to inherited thinking plus the available safe-built-in tool defaults. Compatibility: when the nested `config.json` is absent, a valid root-level `pi-plan-mode.json` or legacy `plan-mode.json` is migrated without overwriting a concurrently created nested file. The nested file always takes precedence.
...
- The implementation boundary is explicit: Plan mode restores tools before current-session implementation, while new-session implementation uses Pi's replacement-session API and only the fresh context for the exact-plan handoff. The accepted plan remains active across compaction until exit/off or a replacement workflow clears it.
...
```txt
agent/extensions/plan-mode/
├── config.json       # Local default tools and thinking level
├── index.ts          # Auto-discovered wrapper
├── src/
│   ├── index.ts      # Extension entrypoint
│   ├── plan-mode.ts  # Extension registration and mode state
│   └── *.ts          # Prompt, policy, question, state, and presentation modules
├── test/             # Upstream-derived and local regression coverage
├── README.md
└── LICENSE
```

The wrapper forwards to `src/index.ts`. The shared TUI source lives at
`agent/extensions/_shared/packages/tui-kit/` and is linked by `agent/package.json`.

...
const SETTINGS_FILE = join("extensions", "plan-mode", "config.json");
...
test("missing and empty default tool settings remain distinct", async () => {
	await withAgentDir(async (agentDir) => {
		...
		await writeFile(join(agentDir, SETTINGS_FILE), JSON.stringify({ defaultPlanTools: [] }));
...
async function withAgentDir(run: (agentDir: string) => Promise<void>) {
	...
	try {
		await mkdir(join(agentDir, "extensions", "plan-mode"), { recursive: true });
		await run(agentDir);
	} finally {
